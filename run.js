...
(async () => {
  ...
  program
    ...
    .option(
      '-l, --language <language>',
      'Recognition language or comma-separated list of languages, defaults to "English"'
    )
    .option(
      '-e, --export-format <format>',
      'Output format. One of: txt (default), txtUnstructured, rtf, docx, xlsx, pptx, pdfa, pdfSearchable, pdfTextAndImages, xml'
    )
    .option(
      '-c, --custom-options <options>',
      "Other custom options passed to REST-ful call,  like 'profile=documentArchiving'"
    )
    .option(
      '-o, --output-path <path>',
      'The path to which to save the processed files'
    )
    .option(
      '-F, --filenames',
      'Output the filenames of the processed and downloaded files'
    );
  // .action(processFiles);
  ...
  program
    .option(
      '-u, --service-url <url>',
      'The http endpoint of the Cloud OCR Service'
    )
  ...
  async function list(options) {
    const ocr = createClient(options);
    const { tasks } = await (options.finished
      ? ocr.listFinishedTasks()
      : ocr.listTasks());
    let result = tasks;
    if (options.summary) {
