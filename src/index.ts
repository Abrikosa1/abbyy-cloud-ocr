...
export class AbbyyOcr {
  ...
  private async callService(
    verb: HTTP_VERBS,
    methodName: string,
    params: KeyStringValueMap = {},
    body?: string
  ) {
    let url = `${this.serviceUrl}/v2/${methodName}?${querystring.stringify(
      params
    )}`;
    const auth = Buffer.from(
      this.appId + ':' + this.password,
      'utf-8'
    ).toString('base64');
    const options: any = {
      method: verb,
  ...
  async process(
    ...
    settings?: typeof ProcessingSettings
  ): Promise<string[]> {
    ...
    let taskData: typeof TaskData = await new Promise((resolve, reject) => {
      this.ocrsdk.processImage(
        filePath,
        this.settings,
        (error: Error | null, taskData: typeof TaskData) => {
          if (error) {
            reject(error);
  ...
  public async *downloadResult(targetDir?: string): AsyncGenerator<string> {
    ...
    const extensions = this.settings.exportFormat
      .split(',')
